...
import { getUserByUsername, getUserPosts, getUserComments, removeSiteAdmin, makeSiteAdmin } from '@/lib/api'
	...
	async function handleToggleAdmin() {
		...
		try {
			if (profile.role === 'Admin') {
				await removeSiteAdmin(profile.id)
			} else {
				await makeSiteAdmin(profile.id)
			}
		} catch (err) {
		...
		<main className="page-wrap px-4 py-10">
			...
			<div className="island-shell rise-in relative mb-6 overflow-hidden rounded-[2rem] px-8 py-10">
				...
				<div className="relative flex items-center gap-5">
					...
					<div>
						<h1 className="display-title flex items-center gap-3 text-2xl font-bold text-[var(--sea-ink)]">
							u/{username}
							{profile?.role === 'admin' && (
								<span className="rounded bg-purple-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-purple-600">
									Site Admin
								</span>)}
						</h1>
						{profile && (
					...
					<div className="ml-auto">
						{currentUser?.role === 'Admin' && currentUser?.username != username && profile && (
							<button
								onClick={handleToggleAdmin}
								className={`rounded-xl px-5 py-2 text-sm font-bold transition ${profile.role === 'Admin'
									? 'border border-[var(--line)] text-red-500 hover:bg-red-500/10'
									: 'bg-[var(--surface-string)] text-[var(--lagoon-deep)] hover:bg-[var(--lagoon)] hover:text-white'
									}`}
							>
								{profile.role === 'Admin' ? 'Remove Site Admin' : 'Make Site Admin'}
							</button>
						)}
					</div>
				</div>
